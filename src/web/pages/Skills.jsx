...
const generateSkillCode = (skillName, prompt) => {
  const templates = {
    'MD → HTML': `// 💪 码坚强 - ${skillName} 技能模板\n// ...`,
    '卡片追光': `// 💪 码坚强 - ${skillName} 技能模板\nimport { useCallback, useRef } from 'react';\nexport default function useCardGlow() { ... }`,
    '矩阵背景': `// 💪 码坚强 - ${skillName} 技能模板\nimport HexBackground from './components/HexBackground';\n// ...`,
  };
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 6 }}>
              ...
              <Card className="glass-card-hover" sx={{ height: '100%' }}>
                <CardActionArea onClick={() => handleSkillClick(skill)} sx={{ height: '100%' }}>
                  <CardContent sx={{ p: 3, display: 'flex', flexDirection: 'column', height: '100%' }}>
                    ...
                    <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.4)', display: 'flex', alignItems: 'center', gap: 0.5 }}>📋 点击生成模板 →</Typography>
                  </CardContent>
                </CardActionArea>
              </Card>
            </GlowCard>
      ...
      <Dialog open={dialogOpen} onClose={() => { setDialogOpen(false); setSelectedSkill(null); }} maxWidth="md" fullWidth
        PaperProps={{ sx: { background: 'rgba(18, 18, 42, 0.98)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 3 } }}>
        ...
        <DialogContent dividers sx={{ borderColor: 'rgba(255,255,255,0.06)' }}>
          <Box component="pre" sx={{ background: '#0a0a1e', borderRadius: 2, p: 2.5, fontSize: '0.8rem', lineHeight: 1.6, color: '#e0e0e0', overflow: 'auto', maxHeight: 480, fontFamily: '"Fira Code", "JetBrains Mono", Consolas, monospace', whiteSpace: 'pre-wrap', wordBreak: 'break-all', border: '1px solid rgba(255,255,255,0.05)' }}>{generatedCode}</Box>
        </DialogContent>
        <DialogActions sx={{ px: 3, py: 2, justifyContent: 'space-between' }}>
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            <Button variant="outlined" onClick={() => window.open('https://www.skillhub.cn', '_blank')} endIcon={<OpenInNewIcon />} sx={{ borderColor: 'rgba(255,255,255,0.2)', color: 'text.secondary' }}>Skill Hub</Button>
