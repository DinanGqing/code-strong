                ...
                <ListItem key={f.id} sx={{ '&:hover': { bgcolor: 'rgba(255,255,255,0.05)' } }}
                  secondaryAction={
                    <Box>
                      ...
                      <IconButton onClick={function() { removeFriend(f.id); }} sx={{ color: 'rgba(255,255,255,0.3)' }}><DeleteIcon /></IconButton>
                    </Box>
                  }>
                  <ListItemAvatar>
                    <Badge badgeContent={f.unread_count} color="error">
                      <Avatar src={f.avatar}>{f.username?.[0]}</Avatar>
                    </Badge>
                  </ListItemAvatar>
                  <ListItemText
              ...
              return (
                <Card key={req.id} sx={{ mb: 1, bgcolor: 'rgba(255,255,255,0.03)' }}>
                  <CardContent sx={{ display: 'flex', alignItems: 'center', gap: 2, py: 1.5 }}>
                    <Avatar src={req.avatar}>{req.username?.[0]}</Avatar>
                    <Box sx={{ flex: 1 }}>
          ...
          <>
            ...
            <List sx={{ bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 2, mb: 3 }}>
              {myChannels.map(function(ch) {
                const leaveBtn = (
                  <IconButton onClick={function(e) { e.stopPropagation(); leaveChannel(ch.id); }} size="small" sx={{ color: 'rgba(255,255,255,0.3)' }}>
      ...
      <Dialog open={createOpen} onClose={function() { setCreateOpen(false); }} maxWidth="xs" fullWidth
        ...
        <DialogContent>
          <TextField fullWidth size="small" label="频道名称" value={channelName}
            ...
            sx={{ mb: 2, '& input': { color: 'white' }, '& label': { color: 'rgba(255,255,255,0.5)' } }} />
