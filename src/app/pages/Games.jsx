  ...
  return (
    <Box sx={{ position: 'relative' }}>
      <Box sx={{ display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap', justifyContent: 'center' }}>
        {[{key:'←→',label:'移动'},{key:'↑',label:'旋转'},{key:'↓',label:'加速'},{key:'空格',label:'硬降'},{key:'P',label:'暂停'}].map(k => (<Chip key={k.key} label={`${k.key} ${k.label}`} size="small" sx={{ background:'rgba(0,212,255,0.08)',color:'rgba(26,26,46,0.55)',fontSize:'0.65rem',height:22,border:'1px solid rgba(255,255,255,0.08)'}}/>))}
      </Box>
...
function SnakeGame() {
  ...
  return (<Box>
    <Box sx={{display:'flex',gap:1,mb:2,flexWrap:'wrap',justifyContent:'center'}}>{[{key:'←↑↓→',label:'方向'},{key:'P',label:'暂停'}].map(k=><Chip key={k.key} label={`${k.key} ${k.label}`} size="small" sx={{background:'rgba(0,255,136,0.08)',color:'rgba(26,26,46,0.55)',fontSize:'0.65rem',height:22,border:'1px solid rgba(255,255,255,0.08)'}}/>)}</Box>
    <Box sx={{display:'flex',justifyContent:'center',gap:2}}>
...
function Game2048() {
  ...
  return(<Box><Box sx={{display:'flex',gap:1,mb:2,flexWrap:'wrap',justifyContent:'center'}}>{[{key:'←↑↓→',label:'滑动'},{key:'触摸',label:'滑动支持'}].map(k=><Chip key={k.key} label={`${k.key} ${k.label}`} size="small" sx={{background:'rgba(155,89,182,0.08)',color:'rgba(26,26,46,0.55)',fontSize:'0.65rem',height:22,border:'1px solid rgba(255,255,255,0.08)'}}/>)}</Box>
    ...
    <Container maxWidth="lg" sx={{py:6}}>
      ...
      <Grid container spacing={3}>{filteredGames.map((g)=>(
        <Grid item xs={12} sm={6} lg={3} key={g.id}><Card className="glass-card-hover" sx={{height:'100%',opacity:g.comingSoon?0.6:1}}><CardActionArea onClick={()=>handleGameClick(g)} sx={{height:'100%'}} disabled={g.comingSoon}><CardContent sx={{p:3,display:'flex',flexDirection:'column',height:'100%'}}>
          ...
          <Typography variant="caption" sx={{color:'rgba(26,26,46,0.45)',display:'flex',alignItems:'center',gap:0.5}}>👥 {g.players.toLocaleString()} 人已游玩 · {g.comingSoon?'即将上线 →':'点击开始 →'}</Typography>
