  ...
  return (
    <Container maxWidth="md" sx={{ py: 6 }}>
      <Card className="glass-card" sx={{ mb: 4 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 3, pt: 3, pb: 2, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <IconButton onClick={() => window.history.back()} sx={{ color: '#00D4FF' }}>
            <ArrowBackIcon />
          </IconButton>
          <Typography variant="h6" sx={{ fontWeight: 700 }}>账户设置</Typography>
        </Box>
        <CardContent sx={{ px: 3, py: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          <Typography variant="subtitle1" sx={{ fontWeight: 600, color: 'rgba(26,26,46,0.85)' }}>基本信息</Typography>
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={3} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
            {(() => {
              const avatarEmoji = form.avatar && !form.avatar.startsWith('http') ? form.avatar : null;
              const presetMatch = avatarEmoji ? PRESET_AVATARS.find(a => a.emoji === avatarEmoji) : null;
              return (
                <Avatar
                  src={form.avatar?.startsWith('http') ? form.avatar : undefined}
                  sx={{ width: 100, height: 100, background: presetMatch?.bg || '#00B4D8', fontSize: '2.8rem' }}
                >
                  {avatarEmoji || form.username?.charAt(0)?.toUpperCase() || '?'}
                </Avatar>
              );
            })()}
